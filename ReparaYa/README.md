...
```mermaid
flowchart TB
    Cliente["👤 Cliente<br/>Persona que necesita reparar<br/>un celular, tablet o computador"]
    Admin["👤 Técnico / Administrador<br/>Gestiona citas y<br/>órdenes de servicio"]

    Sistema["🖥️ Repara Ya<br/>[Sistema de software]<br/>Permite agendar citas de diagnóstico<br/>y gestionar órdenes de servicio"]

    Correo["✉️ Servicio de correo<br/>[Sistema externo]<br/>Envía confirmaciones<br/>y notificaciones"]

    Cliente -->|"Consulta servicios y<br/>agenda citas (HTTPS)"| Sistema
    Admin -->|"Gestiona citas y<br/>órdenes (HTTPS)"| Sistema
    Sistema -->|"Envía notificaciones<br/>(API / SMTP)"| Correo
```
...
flowchart TB
    Cliente["👤 Cliente"]
    Admin["👤 Técnico / Administrador"]

    subgraph Sistema["Repara Ya"]
        Frontend["🌐 Frontend Web<br/>[HTML5, CSS3, JavaScript]<br/>Interfaz para clientes<br/>y administradores"]
        Backend["⚙️ API Backend<br/>[Node.js + Express]<br/>Procesa solicitudes y<br/>expone servicios REST"]
        DB["🗄️ Base de datos<br/>[PostgreSQL]<br/>Almacena clientes, citas,<br/>dispositivos y órdenes"]
    end

    Correo["✉️ Servicio de correo<br/>[SendGrid]<br/>Envío de notificaciones"]

    Cliente -->|HTTPS| Frontend
    Admin -->|HTTPS| Frontend
    Frontend -->|"HTTPS / JSON"| Backend
    Backend -->|"SQL / TCP"| DB
    Backend -->|"API / SMTP"| Correo
```
